...
- настройка иконки и размера окна;
...
Здесь задаётся название выходного пакета, выбирается папка экспорта, запускается сборка и отображается подробный процесс выполнения.
...
- Linux `deb`: собирается в Docker-контейнере Debian, чтобы не зависеть от дистрибутива хоста;
- Linux `rpm`: пока использует fallback-экспорт bundle, полноценная упаковка запланирована;
